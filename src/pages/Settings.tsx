...
const Settings: React.FC = () => {
  ...
  const [isTokenVisible, setIsTokenVisible] = useState<boolean>(false); // State for toggling bearerToken visibility
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        const settings = await window.electron.fetchSettings();
        setProfile((prevProfile) => ({
          ...prevProfile,
          automaticThreatResponse: settings?.automaticThreatResponse || false,
          selectedOption: settings?.selectedOption || 'option1',
    ...
    const fetchWhitelist = async () => {
      try {
        const whitelist = await window.electron.fetchWhitelist();
    ...
    if (file) {
      ...
      reader.onload = (upload) => {
        if (upload.target?.result) {
          setProfile((prevProfile) => ({
            ...prevProfile,
            avatar: upload.target.result as string,
          }));
        }
      };
  ...
  const handleCancel = () => {
    toast.info('Settings update canceled.');
    console.log('Cancel');
  };

  const handleAddIP = async () => {
    try {
      const newEntry = await window.electron.addToWhitelist(newIP);
  ...
  const handleRemoveIP = async (id: number) => {
    try {
      await window.electron.removeFromWhitelist(id);
  ...
  const generateToken = () => {
    ...
    toast.success('New bearer token generated!');
  };
      ...
      <div className="mx-auto max-w-270">
        ...
        <div className="grid grid-cols-5 gap-8">
            ...
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              ...
              <div className="p-7">
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-5.5">
                    <label className="mb-3 block text-sm font-medium text-black dark:text-white" htmlFor="automaticThreatResponse">Automatic Threat Response</label>
                    <input
                      type="checkbox"
                  ...
                  <div className="mb-5.5">
                    <label className="mb-3 block text-sm font-medium text-black dark:text-white" htmlFor="selectedOption">Options</label>
                    <select
                      name="selectedOption"
                      id="selectedOption"
                    ...
                    >
                      ...
                      <option value="option1">Auto Drop / Block all malicious detected packets. Stop all traffic and only allow whitelisted traffic.</option>
                      <option value="option2">Auto Drop / Block all malicious detected packets. Allow all other traffic that's non malicious.</option>
                      <option value="option3">Notify of Malicious Packets only.</option>
                    </select>
                  </div>
                  <div className="mb-5.5">
                    <label className="mb-3 block text-sm font-medium text-black dark:text-white" htmlFor="bearerToken">Bearer Token</label>
                    <div className="relative">
                      <input
                        className="w-full rounded border border-stroke bg-gray py-3 px-4.5 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
                        type={isTokenVisible ? 'text' : 'password'} // Conditionally render the input type
                        name="bearerToken"
                        id="bearerToken"
                      ...
                      <button
                        type="button"
                        className="absolute right-4.5 top-3.5 text-sm text-white bg-blue-600 py-1 px-2 rounded flex items-center" // Add flex and items-center for alignment
                      ...
                      <button
                        type="button"
                        className="absolute right-24 top-3.5 text-sm text-white py-1 px-4 rounded flex items-center bg-transparent" // Set background to transparent
                        onClick={toggleTokenVisibility}
                      >
                        <FontAwesomeIcon icon={isTokenVisible ? faEyeSlash : faEye} /> {/* Only the icon, no text */}
                      </button>
                  ...
                  <div className="flex justify-end gap-4.5">
                    <button
                      className="flex justify-center rounded border border-stroke py-2 px-6 font-medium text-black hover:shadow-1 dark:border-strokedark dark:text-white px-2 rounded ml-2"
                      type="button"
          ...
          <div className="col-span-5 xl:col-span-2">
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              <div className="border-b border-stroke py-4 px-7 dark:border-strokedark">
                <h3 className="font-medium text-black dark:text-white">Whitelist</h3>
              </div>
              <div className="p-7">
                <div className="flex flex-col gap-5.5">
                  <div className="flex justify-between items-center">
                    ...
                    />
                    <button
                      className="ml-4 bg-blue-600 text-white py-2 px-4 rounded"
                      onClick={handleAddIP}
                    >
                      Add
                    </button>
                  ...
                  <div className="whitelist-container">
                    {whitelist.map((entry) => (
                      <div key={entry.id} className="whitelist-entry flex justify-between items-center">
                        <span>{entry.ip_address}</span>
                        <button
                          className="ml-4 bg-red-600 text-white py-1 px-2 rounded"
